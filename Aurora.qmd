...
# Updating Data - This is currently failing
...
```{r}
# I think the MFI is based on .fcs files recorded when running QC beads - MFI not working
...
TheDate <- sub(" .*", "",Gain_Aurora$DateTime[1])
```
...
#RepairAurora <- Data %>% filter(instrument %in% "Aurora")
```
...
PDFPlots <- c(UltraVioletPlotsGain, VioletPlotsGain, BluePlotsGain, YellowGreenPlotsGain, RedPlotsGain, ScatterPlotsGain, LaserDelayPlotsGain, LaserPowerPlotsGain,  ScalingPlotsGain, UltraVioletPlotsRCV, VioletPlotsRCV, BluePlotsRCV, YellowGreenPlotsRCV, RedPlotsRCV, ScatterPlotsRCV)

Filename <- paste0("QCPlots_Aurora")

PDF <- Utility_Patchwork(x=PDFPlots, filename=Filename, returntype="pdf", outfolder=MainFolder, thecolumns=1)
```

## {.sidebar}
Dashboard data  for the **Cytek Aurora** last updated on **`r TheDate`**
...
This dashboard contains the visualized QC data for the cytometers at the [UNC Flow Core](https://www.med.unc.edu/flowcytometry)

To request flow services or book instrument time, visit our [iLabs website](https://uncch.ilab.agilent.com/service_center/3267/?tab=about)

This dashboard was adopted from code written and maintained by [David Rach](https://github.com/DavidRach) using [Quarto](https://quarto.org/) 
